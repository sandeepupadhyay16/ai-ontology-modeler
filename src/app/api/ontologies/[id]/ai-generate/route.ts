...
/**
 * @deprecated Stage 5 CUTOVER (docs/worklog/STAGE-5B.md): `ChatPanel`'s send path now goes
 * through `POST /api/sessions/[id]/turns` + the human review gate + `POST
 * /api/sessions/[id]/promote` (src/lib/promotion.ts) instead of this route. This handler
 * writes directly to the live graph with no staging/review step and deletes concepts that
 * are merely absent from the latest LLM output (see the delete-by-omission logic further
 * down) — exactly what Stage 5 replaced. Left in the codebase, UNWIRED from the primary
 * conversational flow, only because the bulk objective-onboarding wizard
 * (src/app/page.tsx, the `fetch(.../ai-generate` call around line ~880) still calls it for
 * one-shot initial ontology seeding; that call site is out of scope for this cutover. Do
 * not wire any new UI to this route — use the turns/review/promote pipeline instead.
 */
