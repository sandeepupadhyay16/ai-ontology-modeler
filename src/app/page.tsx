  ...
  const loadOntologyData = async (ontoId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // Stage 5's promotion service auto-creates per-module Layer 1 anchor concepts as pure
        // hierarchy infrastructure (see isTagRootConcept) — hide them from the graph viz the
        // same way qualityEvaluator already excludes them from metrics.
        setConcepts((data.concepts || []).filter((c: any) => !isTagRootConcept(c)));
  ...
  const saveDashboardSuggestions = async () => {
    ...
    try {
      ...
      for (const obj of selectedObjectives) {
        setMappingStatusText(`4/5: Creating empty domain ontologies to model conversationally (${index}/${selectedObjectives.length})...`);
        ...
        // Onboarding cutover: new ontologies are created EMPTY and built conversationally in the
        // AI Modeler (candidate -> review -> promote pipeline), never auto-generated by the
        // deprecated ai-generate route.
        if (!resOnto.ok || !newOnto.id) {
          console.error('Ontology creation failed:', newOnto?.error);
        }
        index++;
      }
  ...
  const handleCreateOntology = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Onboarding cutover: drop straight into the conversational modeler for the new (empty)
        // ontology — ModelerPanel opens on the AI Modeler tab, so the user starts by talking.
        if (data?.id) await loadOntologyData(data.id);
      } else {
        setErrorMessage(data.error);
      }
    } catch (err: any) {
